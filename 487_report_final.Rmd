...
\title{DATA 487 Project: \\ An Investigation Into the Robustness of Semi-supervised Model-based Clustering Using Binary Data With Incorrect Labels} 
...
3.  When clustering is performed using the model-based finite-mixtures method [@mclachlan_2000].
...
\begin{table}[htbp]
\begin{center}
\caption{Example questionnaire matrix with \(n\) respondents and \(p\) questions with binary responses.}
\vspace{.1in}
\label{tab:example_mat}
...
\begin{equation}
L(\boldsymbol{\theta};\boldsymbol{y}) = \prod_{i=1}^n \prod_{j=1}^p
\theta_{ij}^{y_{ij}}(1-\theta_{ij})^{1-y_{ij}}.
\label{eq:L1}
\end{equation}
...
\begin{equation}
\ell(\boldsymbol{\theta};\boldsymbol{y}) = \sum_{i=1}^n \sum_{j=1}^p \left [ y_{ij}\log\theta_{ij} + (1-y_{ij}) \log (1-\theta_{ij})\right].
\label{eq:LL1}
\end{equation}
...
\begin{equation}
L(\boldsymbol{\Omega}, \boldsymbol{\pi}; \boldsymbol{y}) = \prod_{i=1}^n L_i = \prod_{i=1}^n \left[\sum_{r=1}^R\left[\pi_r \prod_{j=1}^p\theta_{rj}^{y_{ij}}(1-\theta_{rj})^{1-y_{ij}} \right] \right]
\label{eq:L2}
\end{equation}
...
\begin{equation}
\ell(\boldsymbol{\Omega}, \boldsymbol{\pi}; \boldsymbol{y}) = \sum_{i=1}^n \log\left[\sum_{r=1}^R\left[\pi_r \prod_{j=1}^p\theta_{rj}^{y_{ij}}(1-\theta_{rj})^{1-y_{ij}} \right]  \right]
\label{eq:LL2}
\end{equation}
...
\begin{equation}
L_C(\boldsymbol{\Omega},\boldsymbol{\pi};\boldsymbol{y},\boldsymbol{z}) = \prod_{i=1}^n \prod_{r=1}^R\left[\pi_r\prod_{j=1}^p\theta_{rj}^{y_{ij}}(1-\theta_{rj})^{1-y_{ij}}\right]^{z_{ir}}
\end{equation}

with the corresponding \eqref{eq:LL2} modified to the complete-data log-likelihood $\ell_C$:

\begin{equation}
\ell_C(\boldsymbol{\Omega},\boldsymbol{\pi};\boldsymbol{y},\boldsymbol{z}) = \sum_{i=1}^n \sum_{r=1}^R z_{ir} \log \left[\pi_r\prod_{j=1}^p \theta_{rj}^{y_{ij}}(1-\theta_{rj})^{1-y_{ij}}\right].
\label{eq:complete_unsuper_ll1}
\end{equation}
...
\begin{equation}
E(z_{ir}) = P(i\in r|\boldsymbol{y}_i)= \frac{P(i \in r , \boldsymbol{y}_i)}{P(\boldsymbol{y}_i)} = \frac{\pi_r \prod_{j=1}^p \theta_{rj}^{y_{ij}}(1-\theta_{rj})^{1-y_{ij}}}{\sum_{h=1}^R \pi_h \prod_{j=1}^p \theta_{hj}^{y_{ij}}(1-\theta_{hj})^{1-y_{ij}}}
...
\begin{table}[htbp]
\begin{center}
\caption{Example data matrix with \(n\) rows and \(p\) columns with binary responses and some existing cluster labels. NA represents no knowledge of cluster assignment.}
\vspace{.1in}
\label{tab:label_mat}
...
& \sum_{i=n_l + 1} ^ {n_l + n_u} \log \left[ \sum_{r=1}^R \pi_r \prod_{j=1}^p \theta_{rj}^{y_{ij}}(1-\theta_{rj})^{1-y_{ij}}\right].
\label{eq:overall_semi_ll}
\end{split}
\end{equation}
...
\begin{split}
     ...
     &\left[ \sum_{i=n_l + 1}^{n_l + n_u} \sum_{r=1}^R z_{ir} \log \pi_r + \sum_{i=n_l + 1}^{n_l + n_u} \sum_{j=1}^p \sum_{r=1}^R z_{ir}\left[y_{ij} \log \theta_{rj} + (1-y_{ij})\log (1-\theta_{rj}) \right] \right].
\label{eq:complete_semi_ll}
\end{split}
\end{equation}
...
E[z_{ir}] = \begin{cases}
I(i = r), & i \in \{1,...,n_l\} \\ 
\frac{\pi_r \prod_{j=1}^p \theta_{rj}^{y_{ij}}(1-\theta_{rj})^{1-y_{ij}}}{\sum_{h=1}^R \pi_h \prod_{j=1}^p \theta_{hj}^{y_{ij}}(1-\theta_{hj})^{1-y_{ij}}}, & i \in \{n_l + 1,...,n_l + n_u\}.
\end{cases}
...
All of the simulated datasets were created with $p=5$ columns, $R=3$ row clusters, but different numbers of rows: $n = (300, 1000, 3000)$. The datasets were generated with equal proportions of rows assigned to each cluster.

\begin{table}[htbp]
\begin{center}
\caption{Simulation combinations of dataset size, \(n\), proportion of data labelled \(m\), and proportion of the \(m\%\) labelled data that are incorrect, \(s\).}
\label{tab:sim_tab}
\vspace{.1in}
\begin{tabular}{c|c|c}
\textbf{n} & \textbf{m (known)} & \textbf{s (wrongly labelled)} \\ \hline
\multirow{6}{*}{300} & \multirow{3}{*}{10\%} & 10\% \\
 &  & 30\% \\
 &  & 50\% \\ \cline{2-3} 
 & \multirow{3}{*}{30\%} & 10\% \\
 &  & 30\% \\
 &  & 50\% \\ \hline
\multirow{6}{*}{1000} & \multirow{3}{*}{10\%} & 10\% \\
 &  & 30\% \\
 &  & 50\% \\ \cline{2-3} 
 & \multirow{3}{*}{30\%} & 10\% \\
 &  & 30\% \\
 &  & 50\% \\ \hline
\multirow{6}{*}{3000} & \multirow{3}{*}{10\%} & 10\% \\
 &  & 30\% \\
 &  & 50\% \\ \cline{2-3} 
 & \multirow{3}{*}{30\%} & 10\% \\
 &  & 30\% \\
...
The labelled data in each of the datasets were selected at random and incorrect labels from the labelled subsets, were also selected at random. As a result, there may be not equal proportions of labelled data that belong to each of the three clusters, despite the true cluster proportions created as equal. True cluster proportions of the data are shown in the \ref{Results} section.
...
True values of the $\{\alpha_r\}$ and $\{\beta_j\}$ parameters were set, following the simulation scenarios proposed by @cui_2025 as:as:
...
Initial parameter estimates $\{\pi_r^{(0)}\}$, were set as the true proportions so that $\{\hat{\pi}_r^{(0)}\} = \{0.333, 0.333,0.333\}$. These parameters are not as essential for the evaluation of the clustering performance, so it is not as necessary to use randomly assigned values.

Initial parameter estimates $\boldsymbol{\hat{\Omega}}^{(0)}$ were generated to be close to the true parameter values. For each parameter in $\boldsymbol{\Omega}$, the initial estimates were generated by randomly sampling from a uniform distribution, where each distribution has a lower limit equal to the true parameter value minus one, and upper limit equal to the true parameter value plus one.

## Clustering Performance Measures

After the model fitting procedure is done for $H = 100$ replicates of each data scenario, the mean and standard deviations for each parameter estimate in $\hat{\boldsymbol{\Omega}}$ and $\{\hat{\alpha}_r\}$ were calculated from the replicates.

These are calculated as follows:

\begin{equation}
\text{Mean}(\hat{\omega}) = \frac{1}{H} \sum_{h=1}^H \hat{\omega}^{(h)}, 
\end{equation} \begin{equation}
\text{s.d}(\hat{\omega}) = \text{s.d}\left(\hat{\omega}^{(h=1)}, \hat{\omega}^{(h=2)}, ..., \hat{\omega}^{(h=100)}\right),
\end{equation}
...
Mean parameter estimates and accuracies were used to judge how good the clustering performance is for each data scenario. The mean parameter estimates from $\hat{\boldsymbol{\Omega}}$ are compared to the true values for the simulated datasets and predicted labels are compared to the true labels to calculate clustering accuracy.

## Statistical Software

All analyses in the study were conducted using the **R** software (version 4.5.1) [@R]. The `optim()` function, using the quasi-Newton method (L-BFGS-B), was chosen to maximise the terms from the complete-data log-likelihood in the M-step.
...
The minimum and maximum values for the cluster proportions within the labelled subsets, for each data scenario are provided as:

\begin{table}[htbp]
\centering
\caption{Mean proportions of clusters represented in replicate datasets}
\label{tab:prop-table}
\begin{tabular}{@{}cccccccc@{}}
\textbf{n} & \textbf{Scenario} & \multicolumn{2}{c}{\(\boldsymbol{\pi_1}\)} & \multicolumn{2}{c}{\(\boldsymbol{\pi_2}\)} & \multicolumn{2}{c}{\(\boldsymbol{\pi_3}\)} \\
 &  & min & max & min & max & min & max \\ \midrule
\multirow{8}{*}{300} & \multicolumn{1}{l}{\(m = 10\%\)} & 0.200 & 0.533 & 0.133 & 0.567 & 0.133 & 0.500 \\
 & \multicolumn{1}{l}{\(m = 30\%\)} & 0.233 & 0.422 & 0.244 & 0.422 & 0.244 & 0.433 \\
 & S1 & 0.167 & 0.567 & 0.167 & 0.533 & 0.100 & 0.500 \\
 & S2 & 0.167 & 0.533 & 0.133 & 0.533 & 0.100 & 0.600 \\
 & S3 & 0.100 & 0.500 & 0.167 & 0.533 & 0.100 & 0.567 \\
 & S4 & 0.222 & 0.467 & 0.211 & 0.467 & 0.233 & 0.467 \\
 & S5 & 0.189 & 0.433 & 0.233 & 0.500 & 0.200 & 0.444 \\
 & S6 & 0.189 & 0.456 & 0.233 & 0.478 & 0.233 & 0.444 \\ \midrule
\multicolumn{1}{l}{\multirow{8}{*}{1000}} & \multicolumn{1}{l}{\(m = 10\%\)} & 0.260 & 0.440 & 0.210 & 0.420 & 0.230 & 0.470 \\
\multicolumn{1}{l}{} & \multicolumn{1}{l}{\(m = 30\%\)} & 0.247 & 0.393 & 0.273 & 0.393 & 0.293 & 0.390 \\
\multicolumn{1}{l}{} & S1 & 0.240 & 0.450 & 0.230 & 0.410 & 0.250 & 0.450 \\
\multicolumn{1}{l}{} & S2 & 0.250 & 0.460 & 0.230 & 0.420 & 0.210 & 0.470 \\
\multicolumn{1}{l}{} & S3 & 0.250 & 0.430 & 0.240 & 0.460 & 0.180 & 0.440 \\
\multicolumn{1}{l}{} & S4 & 0.247 & 0.390 & 0.277 & 0.403 & 0.277 & 0.400 \\
\multicolumn{1}{l}{} & S5 & 0.267 & 0.387 & 0.280 & 0.383 & 0.273 & 0.397 \\
\multicolumn{1}{l}{} & S6 & 0.267 & 0.417 & 0.233 & 0.397 & 0.263 & 0.417 \\ \midrule
\multirow{8}{*}{3000} & \multicolumn{1}{l}{\(m = 10\%\)} & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} \\
 & \multicolumn{1}{l}{\(m = 30\%\)} & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} \\
 & S1 & 0.257 & 0.413 & 0.273 & 0.400 & 0.260 & 0.383 \\
 & S2 & 0.257 & 0.390 & 0.287 & 0.400 & 0.240 & 0.373 \\
 & S3 & 0.267 & 0.417 & 0.270 & 0.397 & 0.260 & 0.407 \\
 & S4 & 0.303 & 0.374 & 0.300 & 0.361 & 0.297 & 0.366 \\
 & S5 & 0.302 & 0.366 & 0.300 & 0.371 & 0.303 & 0.367 \\
 & S6 & 0.293 & 0.378 & 0.294 & 0.367 & 0.279 & 0.374 \\ \bottomrule
\end{tabular}
\end{table}
\begin{table}[htbp]
\centering
\caption{Mean and Standard Deviation of Parameter Estimates}
\label{tab:true_tbl}
\begin{tabular}{@{}ccccccccc@{}}
\textbf{Scenario} & \textbf{Param.} & \textbf{Truth} & \multicolumn{2}{c}{\(\mathbf{n=300}\)} & \multicolumn{2}{c}{\(\mathbf{n=1000}\)} & \multicolumn{2}{c}{\(\mathbf{n=3000}\)} \\
\multicolumn{1}{l}{} & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} & mean & s.d & mean & s.d & mean & s.d \\ \midrule
\multirow{9}{*}{\(m = 10\%\)} & \(\mu\) & 0.000 & -0.321 & 0.029 & -0.325 & 0.016 &  &  \\
 & \(\alpha_1\) & -2.000 & -1.527 & 0.029 & -1.523 & 0.016 &  &  \\
 & \(\alpha_2\) & 0.000 & -0.286 & 0.029 & -0.281 & 0.016 &  &  \\
 & \(\beta_1\) & -2.000 & -1.879 & 0.153 & -1.862 & 0.077 &  &  \\
 & \(\beta_2\) & -1.500 & -1.397 & 0.167 & -1.406 & 0.096 &  &  \\
 & \(\beta_3\) & 0.300 & 0.228 & 0.176 & 0.250 & 0.097 &  &  \\
 & \(\beta_4\) & 1.000 & 0.905 & 0.227 & 0.913 & 0.106 &  &  \\
 & \(\pi_1\) & 0.333 & 0.242 & 0.029 & 0.245 & 0.015 &  &  \\
 & \(\pi_2\) & 0.333 & 0.294 & 0.043 & 0.295 & 0.023 &  &  \\ \midrule
\multirow{9}{*}{\(m = 30\%\)} & \(\mu\) & 0.000 & -0.280 & 0.035 & -0.284 & 0.021 &  &  \\
 & \(\alpha_1\) & -2.000 & -1.568 & 0.035 & -1.564 & 0.021 &  &  \\
 & \(\alpha_2\) & 0.000 & -0.327 & 0.035 & -0.323 & 0.021 &  &  \\
 & \(\beta_1\) & -2.000 & -1.900 & 0.161 & -1.890 & 0.084 &  &  \\
 & \(\beta_2\) & -1.500 & -1.413 & 0.178 & -1.423 & 0.109 &  &  \\
 & \(\beta_3\) & 0.300 & 0.228 & 0.197 & 0.256 & 0.112 &  &  \\
 & \(\beta_4\) & 1.000 & 0.919 & 0.246 & 0.927 & 0.131 &  &  \\
 & \(\pi_1\) & 0.333 & 0.266 & 0.019 & 0.268 & 0.012 &  &  \\
 & \(\pi_2\) & 0.333 & 0.310 & 0.026 & 0.310 & 0.018 &  &  \\ \bottomrule
\end{tabular}
\end{table}
\begin{table}[htbp]
\begin{center}
\caption{Mean and Standard Deviation of Parameter Estimates}
\vspace{.1in}
\label{tab:mean-sd-123}
\begin{tabular}{ccccccccc}
\textbf{Scenario} & \textbf{Param.} & \textbf{Truth} & \multicolumn{2}{c}{\(\mathbf{n=300}\)} & \multicolumn{2}{c}{\(\mathbf{n=1000}\)} & \multicolumn{2}{c}{\(\mathbf{n=3000}\)} \\
\multicolumn{1}{l}{} & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} & mean & s.d & mean & s.d & mean & s.d \\ \hline
\multirow{9}{*}{\begin{tabular}[c]{@{}c@{}}Scenario 1:\\ \\ \((m = 10\%, s = 10\%)\)\end{tabular}} & \(\mu\) & 0.000 & -0.330 & 0.030 & -0.337 & 0.016 & -0.336 & 0.010 \\
 & \(\alpha_1\) & -2.000 & -1.520 & 0.030 & -1.512 & 0.016 & -1.512 & 0.010 \\
 & \(\alpha_2\) & 0.000 & -0.270 & 0.030 & -0.270 & 0.016 & -0.271 & 0.010 \\
 & \(\beta_1\) & -2.000 & -1.860 & 0.160 & -1.840 & 0.079 & -1.834 & 0.046 \\
 & \(\beta_2\) & -1.500 & -1.380 & 0.170 & -1.387 & 0.096 & -1.389 & 0.056 \\
 & \(\beta_3\) & 0.300 & 0.230 & 0.170 & 0.253 & 0.092 & 0.247 & 0.056 \\
 & \(\beta_4\) & 1.000 & 0.900 & 0.220 & 0.904 & 0.105 & 0.911 & 0.064 \\
 & \(\pi_1\) & 0.333 & 0.240 & 0.030 & 0.244 & 0.015 & 0.243 & 0.009 \\
 & \(\pi_2\) & 0.333 & 0.290 & 0.050 & 0.295 & 0.022 & 0.299 & 0.013 \\ \hline
\multirow{9}{*}{\begin{tabular}[c]{@{}c@{}}Scenario 2:\\ \\ \((m = 10\%, s = 30\%)\)\end{tabular}} & \(\mu\) & 0.000 & -0.358 & 0.031 & -0.360 & 0.018 & -0.359 & 0.011 \\
 & \(\alpha_1\) & -2.000 & -1.490 & 0.031 & -1.488 & 0.018 & -1.490 & 0.011 \\
 & \(\alpha_2\) & 0.000 & -0.249 & 0.031 & -0.247 & 0.018 & -0.248 & 0.011 \\
 & \(\beta_1\) & -2.000 & -1.814 & 0.157 & -1.797 & 0.081 & -1.793 & 0.047 \\
 & \(\beta_2\) & -1.500 & -1.339 & 0.160 & -1.348 & 0.090 & -1.353 & 0.057 \\
 & \(\beta_3\) & 0.300 & 0.237 & 0.165 & 0.256 & 0.086 & 0.254 & 0.054 \\
 & \(\beta_4\) & 1.000 & 0.881 & 0.216 & 0.887 & 0.106 & 0.895 & 0.057 \\
 & \(\pi_1\) & 0.333 & 0.243 & 0.031 & 0.245 & 0.017 & 0.242 & 0.010 \\
 & \(\pi_2\) & 0.333 & 0.289 & 0.043 & 0.294 & 0.025 & 0.300 & 0.014 \\ \hline
\multirow{9}{*}{\begin{tabular}[c]{@{}c@{}}Scenario 3:\\ \\ \((m = 10\%, s = 50\%)\)\end{tabular}} & \(\mu\) & 0.000 & -0.379 & 0.031 & -0.377 & 0.021 & -0.379 & 0.010 \\
 & \(\alpha_1\) & -2.000 & -1.470 & 0.031 & -1.472 & 0.021 & -1.469 & 0.010 \\
 & \(\alpha_2\) & 0.000 & -0.228 & 0.031 & -0.230 & 0.021 & -0.228 & 0.010 \\
 & \(\beta_1\) & -2.000 & -1.773 & 0.152 & -1.774 & 0.083 & -1.759 & 0.047 \\
 & \(\beta_2\) & -1.500 & -1.306 & 0.156 & -1.324 & 0.094 & -1.324 & 0.055 \\
 & \(\beta_3\) & 0.300 & 0.243 & 0.161 & 0.263 & 0.094 & 0.256 & 0.053 \\
 & \(\beta_4\) & 1.000 & 0.873 & 0.195 & 0.877 & 0.104 & 0.878 & 0.054 \\
 & \(\pi_1\) & 0.333 & 0.237 & 0.032 & 0.244 & 0.016 & 0.244 & 0.010 \\
 & \(\pi_2\) & 0.333 & 0.296 & 0.045 & 0.297 & 0.023 & 0.296 & 0.014 \\ \hline
\end{tabular}
\end{center}
\end{table}
...
\caption{Mean and Standard Deviation of Parameter Estimates}
\vspace{.1in}
\label{tab:mean-sd-456}
\begin{tabular}{ccccccccc}
\textbf{Scenario} & \textbf{Param.} & \textbf{Truth} & \multicolumn{2}{c}{\(\mathbf{n=300}\)} & \multicolumn{2}{c}{\(\mathbf{n=1000}\)} & \multicolumn{2}{c}{\(\mathbf{n=3000}\)} \\
\multicolumn{1}{l}{} & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} & mean & s.d & mean & s.d & mean & s.d \\ \hline
\multirow{9}{*}{\begin{tabular}[c]{@{}c@{}}Scenario 4:\\ \\ \((m = 30\%, s = 10\%)\)\end{tabular}} & \(\mu\) & 0.000 & -0.317 & 0.040 & -0.326 & 0.022 & -0.324 & 0.013 \\
 & \(\alpha_1\) & -2.000 & -1.531 & 0.040 & -1.522 & 0.022 & -1.524 & 0.013 \\
 & \(\alpha_2\) & 0.000 & -0.290 & 0.040 & -0.281 & 0.022 & -0.283 & 0.013 \\
 & \(\beta_1\) & -2.000 & -1.831 & 0.166 & -1.805 & 0.078 & -1.806 & 0.052 \\
 & \(\beta_2\) & -1.500 & -1.351 & 0.183 & -1.346 & 0.104 & -1.356 & 0.058 \\
 & \(\beta_3\) & 0.300 & 0.240 & 0.188 & 0.267 & 0.103 & 0.264 & 0.057 \\
 & \(\beta_4\) & 1.000 & 0.897 & 0.220 & 0.903 & 0.119 & 0.904 & 0.076 \\
 & \(\pi_1\) & 0.333 & 0.268 & 0.024 & 0.268 & 0.013 & 0.270 & 0.007 \\
 & \(\pi_2\) & 0.333 & 0.309 & 0.031 & 0.311 & 0.019 & 0.308 & 0.010 \\ \hline
\multirow{9}{*}{\begin{tabular}[c]{@{}c@{}}Scenario 5:\\ \\ \((m = 30\%, s = 30\%)\)\end{tabular}} & \(\mu\) & 0.000 & -0.382 & 0.039 & -0.389 & 0.023 & -0.390 & 0.011 \\
 & \(\alpha_1\) & -2.000 & -1.467 & 0.039 & -1.460 & 0.023 & -1.459 & 0.011 \\
 & \(\alpha_2\) & 0.000 & -0.225 & 0.039 & -0.218 & 0.023 & -0.217 & 0.011 \\
 & \(\beta_1\) & -2.000 & -1.711 & 0.160 & -1.704 & 0.085 & -1.695 & 0.046 \\
 & \(\beta_2\) & -1.500 & -1.245 & 0.166 & -1.265 & 0.099 & -1.259 & 0.054 \\
 & \(\beta_3\) & 0.300 & 0.252 & 0.172 & 0.264 & 0.091 & 0.271 & 0.054 \\
 & \(\beta_4\) & 1.000 & 0.858 & 0.215 & 0.855 & 0.104 & 0.854 & 0.062 \\
 & \(\pi_1\) & 0.333 & 0.263 & 0.027 & 0.270 & 0.014 & 0.270 & 0.008 \\
 & \(\pi_2\) & 0.333 & 0.316 & 0.034 & 0.308 & 0.019 & 0.307 & 0.011 \\ \hline
\multirow{9}{*}{\begin{tabular}[c]{@{}c@{}}Scenario 6:\\ \\ \((m = 30\%, s = 50\%)\)\end{tabular}} & \(\mu\) & 0.000 & -0.433 & 0.044 & -0.443 & 0.024 & -0.445 & 0.011 \\
 & \(\alpha_1\) & -2.000 & -1.415 & 0.044 & -1.406 & 0.024 & -1.403 & 0.011 \\
 & \(\alpha_2\) & 0.000 & -0.174 & 0.044 & -0.164 & 0.024 & -0.162 & 0.011 \\
 & \(\beta_1\) & -2.000 & -1.644 & 0.160 & -1.631 & 0.078 & -1.615 & 0.052 \\
 & \(\beta_2\) & -1.500 & -1.196 & 0.177 & -1.191 & 0.093 & -1.190 & 0.054 \\
 & \(\beta_3\) & 0.300 & 0.260 & 0.166 & 0.270 & 0.087 & 0.267 & 0.049 \\
 & \(\beta_4\) & 1.000 & 0.821 & 0.185 & 0.808 & 0.100 & 0.804 & 0.061 \\
 & \(\pi_1\) & 0.333 & 0.267 & 0.027 & 0.272 & 0.017 & 0.269 & 0.009 \\
 & \(\pi_2\) & 0.333 & 0.314 & 0.033 & 0.307 & 0.023 & 0.309 & 0.012 \\ \hline
\end{tabular}
\end{center}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Mean and Standard Deviation of Clustering Accuracy}
\label{tab:accuracy}
\begin{tabular}{@{}ccccccc@{}}
\textbf{Scenario} & \multicolumn{2}{c}{\(\mathbf{n=300}\)} & \multicolumn{2}{c}{\(\mathbf{n=1000}\)} & \multicolumn{2}{c}{\(\mathbf{n=3000}\)} \\
\multicolumn{1}{l}{} & mean & s.d & mean & s.d & mean & s.d \\ \midrule
\((m = 10\% \) & 0.674 & 0.047 & 0.693 & 0.019 & \multicolumn{1}{l}{} & \multicolumn{1}{l}{} \\
\begin{tabular}[c]{@{}c@{}}Scenario 1:\\ \((m = 10\%, s = 10\%)\)\end{tabular} & 0.672 & 0.052 & 0.692 & 0.023 & 0.696 & 0.006 \\
\begin{tabular}[c]{@{}c@{}}Scenario 2:\\ \((m = 10\%, s = 30\%)\)\end{tabular} & 0.677 & 0.049 & 0.690 & 0.027 & 0.696 & 0.006 \\
\begin{tabular}[c]{@{}c@{}}Scenario 3:\\ \((m = 10\%, s = 50\%)\)\end{tabular} & 0.673 & 0.049 & 0.690 & 0.026 & 0.696 & 0.006 \\
\((m = 30\% \) & 0.692 & 0.033 & 0.693 & 0.016 &  &  \\
\begin{tabular}[c]{@{}c@{}}Scenario 4:\\ \((m = 30\%, s = 10\%)\)\end{tabular} & 0.686 & 0.041 & 0.693 & 0.016 & 0.696 & 0.007 \\
\begin{tabular}[c]{@{}c@{}}Scenario 5:\\ \((m = 30\%, s = 30\%)\)\end{tabular} & 0.682 & 0.047 & 0.693 & 0.016 & 0.696 & 0.007 \\
\begin{tabular}[c]{@{}c@{}}Scenario 6:\\ \((m = 30\%, s = 50\%)\)\end{tabular} & 0.688 & 0.039 & 0.693 & 0.016 & 0.696 & 0.007 \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{center}
\includegraphics[]{m10s10.pdf}
\captionof{figure}{Results for m = 10\%, s = 10\%.}
\label{fig:m10s10}
\end{center}
...
\begin{center}
\includegraphics[]{m10s30.pdf}
\captionof{figure}{Results for m = 10\%, s = 30\%.}
\label{fig:m10s30}
\end{center}
...
\begin{center}
\includegraphics[]{m10s50.pdf}
\captionof{figure}{Results for m = 10\%, s = 50\%.}
\label{fig:m10s50}
\end{center}
...
\begin{center}
\includegraphics[]{m30s10.pdf}
\captionof{figure}{Results for m = 30\%, s = 10\%.}
\label{fig:m30s10}
\end{center}
...
\begin{center}
\includegraphics[]{m30s30.pdf}
\captionof{figure}{Results for m = 30\%, s = 30\%.}
\label{fig:m30s30}
\end{center}
...
\begin{center}
\includegraphics[]{m30s50.pdf}
\captionof{figure}{Results for m = 30\%, s = 50\%.}
\label{fig:m30s50}
\end{center}
...
Overall, this research project aimed to investigate the impact of using incorrect labels for data in semi-supervised model-clustering, using binary mixture models.

-   Brief summary from results

if the clustering manages to pick up on if incorrect labels should actually be recorrected -\> another Expectation step

For simplicity purposes, initial parameter estimates were set using random values, instead of using algorithms to set the values. @cui_2024 initialised their starting parameter values using .... This provides more reliable starting estimates, rather than random initialisation, which may help for quicker convergence and to exit out of local minima of the data.

In our study, we simulate datasets where each of the clusters are present in the data, with one third of the observations assigned to each cluster. There may be situations where observed data may not be available for one or more more clusters, so furthering the simulation study with data scenarios where one or more clusters may be absent from the known subset of the data, is one way this study can be extended.

Time and computational limitations were frequent and needed to be considered, so this meant that precision of the EM algorithm was decreased for quicker convergence of model fitting. In future, with more time and computational power, precision of the EM algorithm would be increased, likely impacting the clustering accuracy and parameter estimates.
...
\begin{table}[htbp]
\centering
\caption{Mean time in seconds to run the EM algorithm for each data scenario based on 100 replicated datasets}
\label{tab:time}
\begin{tabular}{@{}clccclccc@{}}
\toprule
\textbf{n} & \(\mathbf{m = 10\%}\) & \textbf{S1} & \textbf{S2} & \textbf{S3} & \(\mathbf{m = 30\%}\) & \textbf{S4} & \textbf{S5} & \textbf{S6} \\ \midrule
300 & \multicolumn{1}{c}{70.8} & 45.2 & 43.4 & 44.0 & \multicolumn{1}{c}{48.7} & 29.0 & 26.7 & 26.6 \\
1000 & \multicolumn{1}{c}{221.2} & 130.6 & 125.8 & 126.6 & \multicolumn{1}{c}{156.6} & 90.6 & 85.3 & 88.8 \\
3000 &  & 366.2 & 368.6 & 367.7 &  & 279.4 & 242.7 & 258.1 \\ \bottomrule
\end{tabular}
\end{table}
\end{appendix}
